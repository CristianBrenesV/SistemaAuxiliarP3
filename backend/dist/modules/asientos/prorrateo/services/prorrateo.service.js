...
exports.guardarProrrateo = exports.obtenerDistribucionTerceros = exports.obtenerDistribucionCC = exports.obtenerLineaDetalle = void 0;
const db_1 = require("../../../../config/db");
const bitacora_service_1 = require("../../../../modules/bitacora/bitacora.service");
const obtenerLineaDetalle = async (idDetalle) => {
    const connection = await db_1.pool.getConnection();
    try {
        const [rows] = await connection.query('SELECT IdAsientoDetalle, Monto FROM asientocontabledetalle WHERE IdAsientoDetalle = ?', [idDetalle]);
        return rows[0];
    }
    finally {
        connection.release();
...
exports.obtenerLineaDetalle = obtenerLineaDetalle;
const obtenerDistribucionCC = async (idDetalle) => {
    const connection = await db_1.pool.getConnection();
    try {
...
exports.obtenerDistribucionCC = obtenerDistribucionCC;
const obtenerDistribucionTerceros = async (idDetalle) => {
    const connection = await db_1.pool.getConnection();
    try {
        const [rows] = await connection.query(`
            SELECT 
...
exports.obtenerDistribucionTerceros = obtenerDistribucionTerceros;
const guardarProrrateo = async (data, idUsuario) => {
    const connection = await db_1.pool.getConnection();
    try {
        ...
        if (!estadoRows.length)
            throw new Error('La línea del asiento no existe.');
        ...
        await (0, bitacora_service_1.registrarBitacora)(idUsuario, es_tercero ? 'Prorrateo Terceros Guardado' : 'Prorrateo Costos Guardado', { id_detalle, total: totalEnviado });
...
exports.guardarProrrateo = guardarProrrateo;
//# sourceMappingURL=prorrateo.service.js.map